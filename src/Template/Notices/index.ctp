<?php $this -> Html -> addCrumb(__('Notice'), array('controller' => 'guest_books', 'action' => 'index')) ?>
...
<section id="sl_notice_index">
	<article class="table-responsive">	
		<table width="100%" cellpadding="0" cellspacing="0" class="table table-striped">
    		...
    		<tr>
    			<td class="sl_t_id"><?php echo $notice['id'] ?></td>
    			<td class="sl_t_title"><?php echo $this -> Html -> link($this->Text->truncate($notice['title'],50), array('controller' => 'notices', 'action' => 'view', $notice['id'])) ?></td>
    			<td class="sl_t_name"><?php echo $notice->user->name ?></td>
       	<!-- <td class="sl_t_count"><?php echo $notice['Notice']['count'] ?></td> -->
       	<td class="sl_t_created_at"><?php echo $notice['created_at'] ?></td>
       </tr>
    	<?php endforeach ?>
    	<?php unset($notices) ?>
    	<?php unset($notice) ?>    		
